...
function esNumeroValido(numeroTelefono) {
  ...
  return Object.keys(PHONENUMBER_MCC).some((codigo) =>
    numeroSinSigno.startsWith(codigo),
  );
}
...
async function start(file) {
  ...
  p.on("exit", (_, code) => {
    isRunning = false;
    console.error(
      chalk.red.bold("[ ERROR ] Ocurrió un error inesperado:"),
      code,
    );
    p.process.kill();
    isRunning = false;
